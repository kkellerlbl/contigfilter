package contigfilter::contigfilterServer;
...
our %async_run_methods = (
        'count_contigs_async' => 'contigfilter.count_contigs',
);

our %async_check_methods = (
        'count_contigs_check' => 'contigfilter.count_contigs',
);
...
sub call_method {
    ...
    my $ctx = contigfilter::contigfilterServerContext->new($self->{loggers}->{userlog},
...
{
    # Service contigfilter requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for contigfilter but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = contigfilter::contigfilterServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service contigfilter.");
    }
...
package contigfilter::contigfilterServerContext;

use strict;

=head1 NAME

contigfilter::contigfilterServerContext
...
package contigfilter::contigfilterServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = contigfilter::contigfilterServer->new(
