package contigfilter::contigfilterClient;
...
contigfilter::contigfilterClient
...
sub new
{
    ...
    my $self = {
	client => contigfilter::contigfilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
	...
	method => "contigfilter.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for contigfilter::contigfilterClient\n";
    }
    if ($sMajor == 0) {
        warn "contigfilter::contigfilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package contigfilter::contigfilterClient::RpcClient;
